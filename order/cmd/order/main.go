...
import (
	...
	"github.com/Ujjwaljain16/E-commerce-Backend/order"
...
func main() {
	...
	log.Info(ctx, "Starting Order Service", nil)

	// Get configuration from environment
	dbURL := getEnv("DATABASE_URL", "postgres://postgres:postgres@localhost:5433/ecommerce?sslmode=disable")
	port := getEnv("PORT", "50053") // Using 50053 for Order Service (Account: 50051, Catalog: 50052)
	...
	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(metrics.UnaryServerInterceptor("order-service")),
	)
	...
	log.Info(ctx, "Order Service listening", map[string]interface{}{
		"port":         port,
